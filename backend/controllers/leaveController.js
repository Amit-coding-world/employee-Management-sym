...
const getLeaves=async(req,res)=>{
    try {
       ...
       return res
      .status(500)
      .json({ success: false, error: "Leaves add server error" }); 
    }
}
const getLeaveDetail=async(req,res)=>{
    try {
      const {id}=req.params;
       ...
       return res
      .status(500)
      .json({ success: false, error: "Leaves detail server error" }); 
    }
}   
...
const updateLeave=async(req,res)=>{
    try {
      ...
      return res.status(200).json({success:true})
       ...
       return res
      .status(500)
      .json({ success: false, error: "Leaves update server error" }); 
    }
}
