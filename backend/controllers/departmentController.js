...
const getDepartments = async (req, res) => {
  try {
    ...
    return res.status(500).json({ success: false, error: error.message || "get departments server error" });
  }
};
...
const updateDepartment = async (req, res) => {
  try {
    ...
    return res.status(500).json({ success: false, error: error.message || 'update department server error' });
  }
};
